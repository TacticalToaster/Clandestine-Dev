...
Clockwork.chatBox:RegisterClass("bug", "ic", function(info)
	local color = Color(66, 245, 173, 255);

	//local localized = Clockwork.chatBox:LangToTable("ChatPlayerRadios", color, info.name, info.text);

	Clockwork.chatBox:Add(info.filtered, nil, color, info.name.." says over the bug", " \""..info.text.."\"");
end);

Clockwork.chatBox:RegisterClass("page", "ic", function(info)
	local color = Color(66, 245, 173, 255);

	//local localized = Clockwork.chatBox:LangToTable("ChatPlayerRadios", color, info.name, info.text);

	Clockwork.chatBox:Add(info.filtered, nil, color, info.name.." pages", " \""..info.text.."\"");
end);