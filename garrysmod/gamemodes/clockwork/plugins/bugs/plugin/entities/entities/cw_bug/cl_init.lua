...
function ENT:HUDPaintTargetID(x, y, alpha)
	...
	local bugType = "Receiver";
	
	if (self:IsTransmitting()) then
		bugType = "Transmitter";
	end

	y = Clockwork.kernel:DrawInfo("Bug "..bugType, x, y, colorTargetID, alpha);
	
	if (tonumber(frequency) == 0) then
		y = Clockwork.kernel:DrawInfo("This bug has no frequency.", x, y, colorWhite, alpha);
	else
